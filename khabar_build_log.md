...
- **2026-08-08 · #15** — **5-brand coverage fix + best-seller section (Report 1).** Diagnosed the 5 missing brands (andora, cizaro, eagle, just_sbr, premoda): they DO have stock data + witnessed stockouts (andora 1,930 witnessed) — they were dropped by the 15-brand `revealed_demand` product table, while P3's `demand_grid` (raw stockout_events) already included them. **Fix:** rewrote `market_undersupply()` to source from `stockout_events` via `demand_grid` (+ blueprint make-more flag where available); `demand_grid` now also returns `brands`. Category×size coverage verified 15→**17 brands** live (shirts·L/M/XL = 17). **Best-seller section:** found `weekly_bestseller_summary` + `bestseller_rank` tables (brands' own best-seller lists w/ weekly ranks). Added `report_lib.bestsellers()` and Report 1 §3 "Proven sellers & low-hanging fruit" = best-sellers by category×subcategory, with the overlap of best-seller × under-supply called out as low-hanging fruit. Live proof: **jeans·wide-baggy is a top best-seller (17) AND under-supplied (588 stockouts)** = textbook low-hanging fruit. All 6 report files compile clean.

- **2026-08-08 · #16** — **VERIFIED LIVE (all).** Pulled 3 reports from repo: R1 has "Proven sellers & low-hanging fruit" section + size table reaching 17 brands (6 cells at 17; coverage fix confirmed) + low-hanging-fruit callout (wide-baggy); R2 "Does colour move price? — shirts" with verdict + EGP colour bars; R3 warming/cooling live (DIRECTIONAL, 11 warming cells). P3 + 5-brand fix + best-seller section all confirmed rendering correctly. **All DONE.**
- **2026-08-08 · #17** — **P4 signal fusion (v1 — the buy board).** Added `report_lib.order_verdicts()`: fuses under-supply (demand_grid) + proven demand (bestsellers) + timing (demand_trend) at category×subcategory into a ranked board with VERDICT (STRONG BUY / BUY / WATCH), why (tags), when (timing). Design choice from live check: liquidation distress is category-level (would wrongly penalise a hot subcategory like short-sleeve shirts because formal/linen shirts are being discounted), so the per-style anti-signal is the subcategory TREND (cooling = fading), not category distress. Verdict logic: proven + not-cooling = STRONG BUY; (proven & cooling) or (unproven & warming) = BUY; else WATCH. Rewrote `report_order.py` to lead with the buy board (subsumes the old subcategory + best-seller sections), then category×size, colours, coverage. This is the "decision + why + when" layer — flags the trap a plain gap report misses (under-supplied but cooling = market retreating). All 6 files compile clean.

- **2026-08-08 · #18** — **Subcategory coverage guard + P4 fusion for Report 4.** Founder flagged subcategory coverage. Measured demand-weighted per category: core apparel 65–80% (shirts 79, t-shirts 76, jeans 78, jackets 75, sweaters 80, trousers 67, shorts 65) but accessories/footwear (bags, swimwear, loungewear, cardigans, tank-tops, sandals, slippers) = 0%. So subcategory is binary-by-category, strong where buys concentrate. Added **coverage guard** to `order_verdicts(min_coverage=0.5)`: only categories with ≥50% subcategory coverage enter the buy board; low-coverage ones stay in the category×size view. Added **`discount_verdicts()`** (category-level fusion: depth + clearance + distress → ACTIVE CLEARANCE / STICKY DISTRESS / HEALTHY MARKDOWN / FIRM) and rewrote `report_discount.py` to lead with the fused discount board (why it's the verdict, distress context), then depth/distress detail + maturing curve. Report 4 fusion is category-level so coverage isn't a factor. All 6 files compile clean.

- **2026-08-09 · #19** — **Taxonomy classifier: Gemini dropped, made recurring.**
  - **Khotwh health check:** 7,888 products (7,883 active), 30k variants. Clean: zero prices 0, delisted-but-in-stock 0, witness rate 86%, FOP 100%. **RED FLAG: 1,420 products (18%) "uncategorized"** = the stale classifier. Subcategory 51%, color_family 91%. Only 9 snapshot days (young).
  - **Root cause of coverage gaps:** `taxonomy_backfill.py` is wired to PlanetScale (`KHABAR_DB_URL`) and works, but the workflow is **manual-only** (no schedule) → new products pile up unclassified. The ~49% subcategory and 18% Khotwh uncategorized are simply backlog from not running.
  - **Discovery:** `product_variants.color_family` already exists (89% populated, 16 clean values from `color_map` dictionary) — my P3 `canonical_color` was re-normalizing raw `color` unnecessarily.
  - **Rewrite:** dropped all Gemini code from `taxonomy_backfill.py` (912→548 lines). `pass_colors` now uses deterministic keyword rules aligned to COLOR_FAMILIES (same dictionary pattern: seeds color_map → classifies → stamps variants). `pass_subcat_text` now uses expanded TITLE_RULES. Removed `pass_subcat_vision`, `pass_audit`, `_init_backend`, `gemini()`, `fetch_image_b64`. All passes are zero-API-call.
  - **Schedule:** new `taxonomy_backfill.yml` runs **Tuesdays 05:00 UTC** (after Monday scraping, before Monday report suite at 12:00). So fresh products classify before they reach the reports.
  - **report_lib:** switched `demand_grid` and `color_price` to read `color_family` from product_variants (with `canonical_color` as fallback for the 11% NULL). Strictly better — uses the classifier's own output.
  - **Dependencies cleaned:** workflow now requires only `psycopg2-binary` (no google-auth, no Pillow, no requests).
  - All 7 files compile clean.

- **2026-08-09 · #20** — **Gemini-free classifier RAN clean (zero AI).** Post-run verified: color_family 89→**93.3%** (23,837 variants stamped deterministically), subcategory ~49→**51.8%** (core apparel strong: t-shirts 88%, jeans 82%, sweaters 77%; womenswear weak: dresses 34%, skirts 40% — TITLE_RULES gap, expandable). Sizes: 283k classified, 35k conservatively queued for review (nothing guessed). **NEW FINDING (correction):** the subcat passes do NOT clear "uncategorized" — that's a category-ASSIGNMENT problem (no category pass; category_normalized set at scrape via 90-row category_map). 6,880 products (Khotwh 1,420) are uncategorized because brands like Khotwh name by season+style ("Women/Summer25/Printed Long-Sleeve", "Men/Henley'23") which exact-match category_map misses. They ARE categorizable by keyword on category_raw/title. **PROPOSED:** deterministic category-inference pass (needs founder's domain mapping: henley→t-shirts, long-sleeve→?, oversize→?). Awaiting founder's category rules before building. — DONE: Gemini removal + weekly schedule. OPEN: category-inference pass for uncategorized.
- **2026-08-09 · #21** — **Category-inference pass added (evidence-based, no founder input needed).** Instead of asking, inferred each style-name's category from the dominant category of already-categorized products containing that word. Used only strong-agreement rules (tunic/blouse/sweatshirt→shirts 98-100%, cardigan→cardigans 95%, pullover→sweaters 93%, tank→tank-tops 90%, vest→vests 80%, long-sleeve→shirts 55-73%, henley→t-shirts 61%); omitted ambiguous fit-words (polo/oversize/crop <55%). Added `pass_category()` to taxonomy_backfill.py (runs FIRST, before subcat passes; matches category_raw OR name; conservative — unmatched stays uncategorized). Wired into dispatch + workflow choices. Compiles clean, no undefined names. Rescues the bulk of the 6,880 uncategorized (incl. Khotwh's long-sleeve/henley) on next run; recurs weekly. **Taxonomy work COMPLETE.**

## Status: P0–P4 DONE · Suite live weekly · Taxonomy: Gemini-free, scheduled, category-inference added. Batch to commit: taxonomy_backfill.py, taxonomy_backfill.yml. → Moving to P5 (commercialize).
